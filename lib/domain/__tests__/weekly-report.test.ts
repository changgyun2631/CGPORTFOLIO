...
const baseInput: WeeklyReportInput = {
  ...
  facts: {
    // 실제 계좌 수치가 아니라 임의값이다 — 이 파일은 커밋된다.
    vsPeakPercent: 80, maxDrawdown: -30, performanceMaxDrawdown: -35, peakAt: "2026-06-30T15:30:00+09:00",
...
describe("buildWeeklyExposureReport", () => {
  ...
  it("관찰에는 사실만 적고 매매 판단은 넣지 않는다", () => {
    const body = buildWeeklyExposureReport(baseInput).body;
    expect(body).toContain("최근 1년 최고점 대비 80.0%");
    expect(body).toContain("최근 1년 최대낙폭 -35.0% (입출금 제거한 성과 기준)");
    expect(body).toContain("계좌 평가액 낙폭 -30.0% (입출금 포함)");
