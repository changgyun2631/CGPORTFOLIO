...
| **실제 거래원장 복원** | 2026-09-16 사용자의 증권사 전체 거래원장(1년치, 5,368행)을 파싱해 부트스트랩 1건짜리 가짜 거래를 실제 매매 1,069건+입출금/환전 790건+배당 68건으로 교체. `buildPositions()`를 실제 실행해 보유수량을 스크린샷과 대조 — 28종목 중 23종목 소수점까지 정확 일치, 5종목은 1~3주 소액 오차. 예수금은 실제값과 정확히 일치(원화·달러 둘 다). 총 평가금액도 스크린샷 대비 0.34% 오차. 실제 금액은 저장소가 Public이라 이 문서에 적지 않는다. 상세는 아래 "실거래 데이터 상태" 참고 |
...
2026-09-15에는 "보유종목 스냅샷" CSV(`계좌.csv`)로 부트스트랩 거래 1건씩만 넣었었다.
2026-09-16에 사용자가 **증권사 전체 거래원장**(1년치, `2110.xlsx`)을 줘서 실제 매매
1,069건 + 실제 입출금/환전 790건 + 실제 배당 68건으로 완전히 교체했다. 상세 처리
규칙과 미해결 사항은 `WORK_ORDER.md` B-1을 볼 것 — 여기서는 검증 결과만 요약한다.

**검증한 것**:
- `buildPositions()`를 Node로 직접 실행해 계산된 보유수량을 사용자의 실시간잔고
  스크린샷(28종목)과 대조 — **23종목 소수점까지 정확 일치** (SCHD 1.4538주 포함).
  ANET/KLAC/PANW/VRT가 1주씩, QLD가 3주 모자란 소액 오차가 남아 있다
- `buildCashBalances()`로 계산한 예수금이 실제와 **정확히 일치**: 원화 0원,
  달러 둘 다. 단, 이 일치는 정직하게 밝혀둔 보정 항목 2건(`cashflows.json`에
  "원장 재구성 보정"으로 표시) 덕분이다 — 카테고리별로 원인을 못 찾은 차이가
  있었다 (정확한 금액은 로컬 `cashflows.json`에서 확인할 것 — 저장소가 Public이다)
- 대시보드 총 평가금액이 스크린샷 대비 0.34% 차이로 근접
- **실현손익이 처음으로 실제 값(양수, 이익)을 보여준다** — 부트스트랩 방식일 때는
  항상 0원이었다
- 배당 화면의 연도 선택기·종목별 누적 막대(오늘 세션에 새로 만든 기능, B-6)가 실제
  68건의 배당 데이터로 정상 렌더링되는 것까지 브라우저로 확인

**QLD 등 5종목 처리 중 발견한 진짜 원인**: 이 계좌는 자동 리밸런싱 같은 걸 쓰는지,
같은 초에 매도가 매수보다 먼저 기록되는 거래가 여러 번 있었다. `buildPositions()`가
`Math.min(tx.shares, position.shares)`로 초과 매도를 클램프하기 때문에, 이 순서로
처리하면 실제로는 없던 보유수량 손실이 생긴다. 같은 날 매수/매도가 섞인 거래는
매수 쪽 타임스탬프를 그날 00:00:00으로 당겨 항상 먼저 처리되게 고쳐서 16개 종목의
오차를 없앴다. 남은 5개는 같은 문제가 더 미묘하게 남아 있을 수 있다.

**아직 안 된 것**:
- 위 5종목의 소액 보유수량 오차, 예수금 보정 항목의 진짜 원인 규명 (`WORK_ORDER.md` B-1)
- 국내 공모주 청약(도우인시스 등)으로 실제 보유 중인 국내 종목은 이 시스템 범위 밖이라
  반영 안 함 (순현금 유출만 출금으로 처리)
- 기존 시드에 있던 `418660`/`0015B0`/`490590` 등 국내 ETF는 완전히 삭제했다 (`491620`만
  `leverage-ladder`/`dca-monthly` 데모 백테스트가 참조해서 심볼 정의만 남겨둠, 보유
  거래는 없음). `TQQQ`도 실거래(2025년, 지금은 청산)가 있어 심볼이 남아 있다
...
| 무상 리워드 주식을 원가 0원으로 기록해 평가손익이 실제보다 크게 부풀려짐 | 계좌로 그냥 입고된("대체입고") 주식을 "공짜니까 원가 0원"으로 단순하게 기록했다. 실제로는 증권사가 입고 시점 기준가로 원가를 매긴다 | 사용자가 "실제 평가손익과 다르다"고 알려줘서 발견했다 — 종목별 평균단가를 원본 스크린샷과 대조하는 검증을 해야 이런 게 걸린다. "무상이니 원가 0원"은 직관적이지만 틀렸다. 원가를 고치면 `buildCashBalances()`가 그 금액만큼 실제로 현금이 나간 것처럼 계산하니, 상쇄용 입금도 같이 넣어야 한다 |
